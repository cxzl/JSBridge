...
public class CxzlAppInterface2 implements ClientInterface {

    @Override
    public void clientProcessing(CxzlWebView webView, String data, CxzlCallback callback) {
        Context context = webView.getContext();
        if(context instanceof Activity){
            Toast.makeText(context,"客户端Interface2收到web端消息" + data,Toast.LENGTH_SHORT).show();
        }
        callback.onCallback(0,"客户端Interface2已处理消息");
    }
}
